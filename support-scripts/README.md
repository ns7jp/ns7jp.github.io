...
| ファイル | 用途 | 使う場面 |
|---|---|---|
| `linux-triage.sh` | 負荷 / メモリ / ディスク / サービス / ログ / ネット / 認証 / FW を1本で確認、テキスト出力 | Linux サーバー「重い」「ログインできない」問い合わせ時の一次切り分け |

### 共通ライブラリ + ユニットテスト

| ファイル | 用途 |
|---|---|
| `lib/Triage-Lib.ps1` | しきい値判定 / 状態集約 / メッセージ切り詰めなどの **純関数化されたヘルパー**（副作用なし） |
| `tests/Triage-Lib.Tests.ps1` | 上記の **Pester ユニットテスト（25ケース）**。GitHub Actions の ubuntu-latest 上 pwsh で実行 |
...
`Triage-Lib.ps1` は副作用のない純関数だけを提供します。Pester でローカル実行すると判定ロジックの差分が即座に分かります。
...
GitHub Actions（[.github/workflows/pwsh-tests.yml](../.github/workflows/pwsh-tests.yml)）では、`support-scripts/` 配下の変更時に Pester + PSScriptAnalyzer が自動で走ります。
