...
**ITエンジニア（運用・サポート〜開発）** を志望する島田則幸のポートフォリオサイトです。自学で取り組んだサーバー監視ダッシュボードを軸に、公共職業訓練（2025年10月〜2026年1月）で学んだ HTML / CSS / JavaScript / Python / PHP を活用した Web アプリ・業務効率化デスクトップアプリを含む計6作品を一覧できる構成にしています。各作品の概要・使用技術・学習ポイント・デモリンク・GitHub リンクへアクセスできます。
...
このサイトは、単に作品リンクを並べるだけではなく、**製造現場で培った業務改善・トラブル対応の経験を踏まえ、IT エンジニア（運用・サポート〜開発）を目指す過程で、どの技術を学び、どのような考え方で作品を作ったかを伝えるためのサイト**です。

閲覧者には、次のような情報が伝わることを目指しています。

| 見てほしいこと | 内容 |
|----------------|------|
| 人物像 | これまでの経歴、職業訓練で学んだ内容、IT エンジニアを目指す背景 |
| スキル | HTML / CSS / JavaScript / Python / PHP / データベースなどの習熟度 |
| 制作物 | 6作品の概要、使い方、使用技術、工夫した点 |
| 学習姿勢 | トラブルに対して原因を調べ、修正し、改善した過程 |
| 連絡先 | GitHub やメールなど、連絡・確認に使える情報 |

初学者向けに言い換えると、このポートフォリオは「自分の学習成果を Web 上で見せる履歴書」のようなものです。履歴書が職歴や資格を伝えるのに対し、このサイトでは実際に作ったページやアプリを通して、コードを書いて形にできることを示しています。
...
1. **Top**
   最初に表示されるページです。ヒーロー画像、短い自己紹介、主要スキル、代表作品への導線を置いています。

2. **About Me**
   これまでの経歴、職業訓練で学んだこと、取得資格を確認できます。

3. **Skills**
   HTML / CSS、Python、PHP、JavaScript、データベースなど、学習した技術をカテゴリ別に整理しています。

4. **Works**
   制作した6作品を詳しく紹介しています。作品画像、概要、使用技術、デモリンク、GitHubリンク、制作時のトラブルと解決過程を掲載しています。
...
| スキルページ | `skills.html` | 学習した技術をカテゴリ別に見せる |
| 作品ページ | `works.html` | 6作品の詳細、デモ、ソースコード、学習ポイントを紹介する |
| 連絡先ページ | `contact.html` | メールや GitHub などの連絡先を掲載する |

### `index.html`

サイトの顔となるトップページです。ファーストビューでは背景画像スライダーとキャッチコピーを表示し、閲覧者に「どんな人のポートフォリオか」を最初に伝えます。その下に、自己紹介・スキル・作品のプレビューを配置し、詳細ページへ移動しやすい導線を作っています。

初学者向けに見るポイントは、`header`、`nav`、`section` などの HTML タグでページを区切り、CSS のクラス名で見た目を調整している点です。

### `me.html`

自己紹介ページです。プロフィール、経歴タイムライン、取得資格などを掲載しています。作品だけでは伝わりにくい人物像や、IT エンジニアへキャリアチェンジする背景を補足する役割があります。

### `skills.html`

スキル一覧ページです。HTML / CSS、Python、PHP、JavaScript、データベースなどをカテゴリ別に整理しています。単に技術名を並べるのではなく、どのような制作物で使ったかが分かるようにしています。
...
| # | 作品名 | 主な技術 | 内容 | リポジトリ |
|---|--------|----------|------|------------|
| ① | サーバー監視ダッシュボード | Python / Flask / psutil / Chart.js | PCやサーバーの状態をブラウザで可視化する監視ツール（自学プロジェクト） | [ns7jp/server-monitor](https://github.com/ns7jp/server-monitor) |
| ② | 定型文管理アプリ | Python / Flet | よく使う文章を保存し、ワンクリックでコピーするデスクトップアプリ | [ns7jp/works](https://github.com/ns7jp/works) |
| ③ | 付箋アプリ | Python / Tkinter | 複数の付箋を作成・保存・復元できるデスクトップアプリ | [ns7jp/works](https://github.com/ns7jp/works) |
| ④ | SNSアプリ「Pulse」 | PHP / SQLite / JavaScript | 感情ムードを選んで投稿するSNS | [ns7jp/pulse](https://github.com/ns7jp/pulse) |
| ⑤ | 掲示板アプリ | PHP / MySQL | ユーザー登録、投稿、返信ができる掲示板 | [ns7jp/post](https://github.com/ns7jp/post) |
| ⑥ | サンプル企業サイト | HTML / CSS / JavaScript | 架空企業のレスポンシブ対応コーポレートサイト | [ns7jp/magic](https://github.com/ns7jp/magic) |
